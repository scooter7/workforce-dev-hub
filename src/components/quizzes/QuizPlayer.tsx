...
function getYoutubeEmbedUrl(url: string): string | null {
  ...
  if (url.includes('/embed/') || url.match(/\.(mp4|webm|ogg)$/i)) return url;
...
export default function QuizPlayer({ quizId, userId, attemptId }: QuizPlayerProps) {
  ...
  useEffect(() => {
    if (currentQuestion) {
      // console.log("QuizPlayer - Rendering currentQuestion:", JSON.stringify(currentQuestion, null, 2));
    }
  }, [currentQuestion]);
  ...
  if (!currentQuestion) { // This guard should prevent accessing currentQuestion if undefined
    return <div className="text-center p-8">Preparing question...</div>;
  }

  // Media Rendering Component defined inside QuizPlayer for encapsulation
  const QuestionMedia = ({ question }: { question: QuizQuestion }) => {
    ...
    return null;
  }; // Semicolon here terminates the const assignment

  // Main Quiz Playing UI (Flashcard) - This is the return statement the error refers to
  return (
